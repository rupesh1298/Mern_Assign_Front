...
export default function Cart() {
  ...
  return (
    <>
      <div className={`fixed right-0 top-0 md:w-[20vw] w-full h-full bg-white rounded-lg p-5 ${active ? "translate-x-0":"translate-x-full"} transition-all duration-300 ease-linear z-50`}>
        <div className='flex justify-between my-6 items-center'>
          <span className='text-2xl font-bold text-gray-700'>My Orders</span>
        ...
        <hr className='text-gray-600 h-5 font-bold' />
       {
        ...
        <div className='absolute bottom-0 my-12'>
          ...
          <button className='bg-green-500 p-3 rounded-lg w-[90vw] md:w-[18vw] mt-4 hover:bg-green-600 font-bold text-white' onClick={handleCheck}>Checkout</button>
        </div></div>):(<div><h3 className='text-2xl font-bold text-gray-700 flex  items-center  '>Your Cart is Empty..!</h3></div>)
       }
        
      </div>
      <FaShoppingCart className={`rounded-full bg-white shadow-md text-[50px] p-3 fixed bottom-5 right-3 md:bottom-10 md:right-8 cursor-pointer ${cart.length > 0 && "animate-bounce delay-500 transition-all"}`}  onClick={()=>setActive(!active)} />
    </>
  )
}
